...
function HomePage() {
  ...
  const [favoriteProductUuids, setFavoriteProductUuids] = useState([])
  ...
  const orderedProducts = useMemo(() => {
    return [...filteredProducts].sort((a, b) => {
      const aIsFavorite = favoriteProductUuids.includes(a.uuid)
      const bIsFavorite = favoriteProductUuids.includes(b.uuid)
  ...
  }, [filteredProducts, favoriteProductUuids])

  const handleToggleFavorite = async (productUuid) => {
    const result = await toggleFavorite(productUuid)

    setFavoriteProductUuids((currentFavorites) => {
      if (result.isFavorite) {
        if (currentFavorites.includes(productUuid)) {
          return currentFavorites
        }

        return [...currentFavorites, productUuid]
      }
        ...
        <section className="mt-8">
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              {isAuthenticated() && favoriteProductUuids.length > 0 && (
            ...
            <p className="font-semibold text-stone-700">
              {orderedProducts.length} PRODUCTOS ENCONTRADOS
            </p>
          </div>
          ...
          {!isLoading && !errorMessage && (
            <ProductList
              ...
              favoriteProductUuids={favoriteProductUuids}
